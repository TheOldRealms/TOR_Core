...
|---|---|---|---|---|---|---|---|
| 0 | `Crafting` | **done** | next | — | — | — | The worked example. Epic 1 merged to `next_update` as PR #32 — **squashed**, so every branch stacked above it needed a manual re-merge. Owns `CraftingCareerHooks` in `Framework/`. Strings unblocked: `tortools` MCP server is live. |
