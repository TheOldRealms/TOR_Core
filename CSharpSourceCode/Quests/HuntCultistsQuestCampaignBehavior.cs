...
namespace TOR_Core.Quests
{
    public class HuntCultistsQuestCampaignBehavior : CampaignBehaviorBase
    {
        ...
        public class HuntCultistsIssue : IssueBase
        {
            ...
            public override TextObject IssueBriefByIssueGiver => new TextObject("{=str_tor_quest_hunt_cultist_issue_brief}As a matter of fact, I have a lead on a potential cultist. A grave accusation that needs investigating.");

            public override TextObject IssueAcceptByPlayer => new TextObject("{=str_tor_quest_hunt_cultist_issue_accept_player}What needs to be done?");

            public override TextObject IssueQuestSolutionExplanationByIssueGiver
            {
                get
                {
                    TextObject textObject = new TextObject("{=str_tor_quest_hunt_cultist_issue_explanation}I need you to travel to {TARGET_SETTLEMENT}. Investigate the local populace and root out any cultists. On successful completion, the order will pay you {REWARD}{GOLD_ICON}.", null);
            ...
            public override TextObject IssueQuestSolutionAcceptByPlayer => new TextObject("{=str_tor_quest_hunt_cultist_accept_player}Consider it done.");

            public override bool IsThereAlternativeSolution => false;

            public override bool IsThereLordSolution => false;

            public override TextObject Title => new TextObject("{=str_tor_quest_hunt_cultist_title}A cultist in our midst");

            public override TextObject Description
            {
                get
                {
                    TextObject textObject = new TextObject("{=str_tor_quest_hunt_cultist_description}Travel to target settlement and root out any cultist who may be hiding there.", null);
                    textObject.SetTextVariable("TARGET_SETTLEMENT", _targetSettlement.EncyclopediaLinkWithName);
                    return textObject;
                }
            }
        ...
        public class HuntCultistsQuest : QuestBase
        {
            ...
            public override TextObject Title => new TextObject("{=str_tor_quest_hunt_cultist_quest_title}A cultist in our midst");
            ...
            {
                OfferDialogFlow = DialogFlow.CreateDialogFlow("issue_classic_quest_start", 100).NpcLine(new TextObject("{=str_tor_quest_hunt_cultist_offer_dialog}Excellent. Do not underestimate the ruinous powers, unwavering vigilance is required on your quest!", null), null, null).Condition(() => Hero.OneToOneConversationHero == QuestGiver).Consequence(OnQuestAccepted).CloseDialog();
                DiscussDialogFlow = DialogFlow.CreateDialogFlow("quest_discuss", 100).NpcLine(new TextObject("{=str_tor_quest_hunt_cultist_discuss_dialog}It was good doing business with you.", null), null, null).Condition(() => Hero.OneToOneConversationHero == QuestGiver).CloseDialog();
                Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 199).NpcLine("{=khorne_cultist_mission}This vessel is mine. Don't interfere with my plans!")
                    ...
                    .PlayerLine("{=str_tor_quest_hunt_cultist_prepare_to_die}Prepare to die!")
                    .Consequence(TurnHostile).CloseDialog());
            }

            protected override void OnTimedOut()
            {
                AddLog(new TextObject("{=str_tor_quest_hunt_cultist_out_of_time_log}You failed to complete the investigation in time. Any potential cultists are surely in the wind now."));
            }
            ...
            private void SettlementEntered(MobileParty party, Settlement settlement, Hero hero)
            {
                if (party == MobileParty.MainParty && settlement == _settlement && !_storyPlayed)
                {
                    ...
                    {
                        InquiryData data = new InquiryData("Village Raided", "{=str_tor_quest_hunt_cultist_village_raided_info}The village is raided, no chance to find any cultists now. Come back when the village is repopulated.", true, false, "OK", null, () => InformationManager.HideInquiry(), null);
            ...
            private void AfterStory(InkStory story)
            {
                ...
                {
                    AddLog(new TextObject("{=str_tor_quest_hunt_cultist_log_updated_success}You were successful in uncovering the cultists."));
                    CompleteQuestWithSuccess();
                }
                else CompleteQuestWithFail(new TextObject("{=str_tor_quest_hunt_cultist_log_updated_fail}The cultists escaped."));
            }
            ...
            private void OnQuestAccepted()
            {
                ...
                var acceptLog = new TextObject("{=str_tor_quest_hunt_cultist_started}You were tasked to travel to {TARGET_SETTLEMENT} and root out any cultist who may be hiding there.");
