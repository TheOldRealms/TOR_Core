			...
			if (!GameNetwork.IsClientOrReplay)
			{
				foreach (StandingPointWithWeaponRequirement standingPointWithWeaponRequirement in this.AmmoPickUpStandingPoints)
				{
					if (standingPointWithWeaponRequirement.HasUser)
					{
						...
						ActionIndexCache currentActionValue = userAgent.GetCurrentAction(1);
						if (!(currentActionValue == act_pickup_boulder_begin))
						{
							...
							else if (!userAgent.SetActionChannel(1, act_pickup_boulder_begin, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true) && userAgent.Controller != AgentControllerType.AI)
							{
					...
					if (!GameNetwork.IsClientOrReplay)
					{
						...
						foreach (StandingPointWithWeaponRequirement standingPointWithWeaponRequirement2 in this._ammoLoadPoints)
						{
							...
							else if (standingPointWithWeaponRequirement2.HasUser)
							{
								...
								ActionIndexCache currentActionValue2 = userAgent2.GetCurrentAction(1);
								if (currentActionValue2 == act_usage_trebuchet_load_ammo && userAgent2.GetCurrentActionProgress(1) > 0.56f)
								{
									EquipmentIndex wieldedItemIndex = userAgent2.GetPrimaryWieldedItemIndex();
							...
							else if (standingPointWithWeaponRequirement2.HasAIMovingTo)
							{
								...
								EquipmentIndex wieldedItemIndex2 = movingAgent.GetPrimaryWieldedItemIndex();
		...
		protected override void OnTickParallel(float dt)
		{
			...
			for (int i = 0; i < base.StandingPoints.Count; i++)
			{
				if (base.StandingPoints[i].HasUser)
				{
					...
					{
						if (base.StandingPoints[i].UserAgent.GetCurrentAction(1) == act_usage_trebuchet_reload_2)
						{
			...
			if (!GameNetwork.IsClientOrReplay)
			{
				...
				{
					ActionIndexCache currentActionValue = base.PilotAgent.GetCurrentAction(1);
					if (base.State == RangedSiegeWeapon.WeaponState.WaitingBeforeProjectileLeaving || base.State == RangedSiegeWeapon.WeaponState.Shooting || base.State == RangedSiegeWeapon.WeaponState.WaitingBeforeReloading)
					{
						...
						else if (currentActionValue != act_usage_trebuchet_shoot && !base.PilotAgent.SetActionChannel(1, act_usage_trebuchet_reload_idle, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true) && base.PilotAgent.Controller != AgentControllerType.AI)
						{
							base.PilotAgent.StopUsingGameObjectMT(true, Agent.StopUsingGameObjectFlags.AutoAttachAfterStoppingUsingGameObject);
						}
					}
					else if (currentActionValue != act_usage_trebuchet_reload && currentActionValue != act_usage_trebuchet_shoot && !base.PilotAgent.SetActionChannel(1, act_usage_trebuchet_idle, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true) && base.PilotAgent.Controller != AgentControllerType.AI)
					{
						base.PilotAgent.StopUsingGameObjectMT(true, Agent.StopUsingGameObjectFlags.AutoAttachAfterStoppingUsingGameObject);
					}
				}
				if (base.State != RangedSiegeWeapon.WeaponState.Reloading)
				{
					foreach (StandingPoint standingPoint in this.ReloadStandingPoints)
					{
						if (standingPoint.HasUser && standingPoint != base.PilotStandingPoint)
						{
							...
							if (!userAgent.SetActionChannel(1, act_usage_trebuchet_reload_2_idle, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true) && userAgent.Controller != AgentControllerType.AI)
							{
			...
			if (state == RangedSiegeWeapon.WeaponState.Reloading)
			{
				for (int j = 0; j < this.ReloadStandingPoints.Count; j++)
				{
					if (this.ReloadStandingPoints[j].HasUser)
					{
						...
						ActionIndexCache currentActionValue2 = userAgent3.GetCurrentAction(1);
						...
						else if (!GameNetwork.IsClientOrReplay)
						{
							...
							if (!userAgent3.SetActionChannel(1, actionIndexCache, false, 0UL, 0f, 1f, -0.2f, 0.4f, this.Skeletons[0].GetAnimationParameterAtChannel(0), false, -0.2f, 0, true) && userAgent3.Controller != AgentControllerType.AI)
							{
