...
namespace TOR_Core.BattleMechanics.SFX
{
    public class PlayerFlyableObjectScript : ScriptComponentBehavior
    {
        ...
        protected override void OnTick(float dt)
        {
            ...
            if (!seated) return;
            if (seated && !animationSkipped2 && Agent.Main.GetCurrentAction(0).GetName().StartsWith("act_stand_up"))
            {
            ...
            if (seated && !animationSkipped && Agent.Main.GetCurrentAction(0).GetName().StartsWith("act_sit_down"))
            {
                var sitAnim = ActionIndexCache.Create("act_sit_1");
                Agent.Main.SetActionChannel(0, sitAnim, true);
