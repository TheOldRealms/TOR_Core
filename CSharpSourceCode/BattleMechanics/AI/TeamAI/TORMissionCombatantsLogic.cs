    ...
    public class TORMissionCombatantsLogic : MissionCombatantsLogic
    {
        ...
        public static TORMissionCombatantsLogic CreateFromInstance(MissionCombatantsLogic missionCombatantsLogic)
        {
            return new TORMissionCombatantsLogic(
                Traverse.Create(missionCombatantsLogic).Field("BattleCombatants").GetValue() as IEnumerable<IBattleCombatant>,
                Traverse.Create(missionCombatantsLogic).Field("PlayerBattleCombatant").GetValue() as IBattleCombatant,
                Traverse.Create(missionCombatantsLogic).Field("DefenderLeaderBattleCombatant").GetValue() as IBattleCombatant,
                Traverse.Create(missionCombatantsLogic).Field("AttackerLeaderBattleCombatant").GetValue() as IBattleCombatant,
                (Traverse.Create(missionCombatantsLogic).Field("TeamAIType").GetValue() as Mission.MissionTeamAITypeEnum?).Value,
                (Traverse.Create(missionCombatantsLogic).Field("IsPlayerSergeant").GetValue() as bool?).Value);
        }
    }
}