...
namespace TOR_Core.Ink
{
    public class InkStory
    {
        ...
        public InkStory(string id, string file)
        {
            if (File.Exists(file))
            {
                ...
                using (var reader = File.OpenText(file))
                {
                    ...
                    {
                        fileHandler = new InkFileHandler()
                    };
        ...
        public void Reset()
        {
            ...
            if (!_story.TryGetExternalFunction("StartQuest", out _))
            {
                _story.BindExternalFunction<string>("StartQuest", StartQuest, false);
            }
            if (!_story.TryGetExternalFunction("CloseStory", out _))
            {
                _story.BindExternalFunction("CloseStory", CloseStory, false);
            }
        }
