    ...
    private void OnMissionEnded(IMission obj)
    {
        ...
        if (battleSettlement != null)
        {
            ...
            if (mission.MissionResult != null && mission.MissionResult.BattleResolved && mission.MissionResult.PlayerVictory)
            {
                ...
                var inq = new MultiSelectionInquiryData("Victory!", new TextObject("{=str_tor_custom_settlement_chaos_portal_victory}You are Victorious! Claim your reward! Select one!").ToString(), list, false, 1, 1, TORTextHelper.GetText("tor_inquiry_ok_text", "OK"), null, OnRewardClaimed, null);
                MBInformationManager.ShowMultiSelectionInquiry(inq);
            }
            else
            {
                var inq = new InquiryData("Defeated!", new TextObject("{=str_tor_custom_settlement_chaos_portal_lose}The enemy proved more than a match for you. Better luck next time!").ToString(), true, false, TORTextHelper.GetText("tor_inquiry_ok_text", "OK"), null, null, null);
