...
namespace TOR_Core.CampaignMechanics.Chaos
{
    public class ChaosCampaignBehavior : CampaignBehaviorBase
    {
        ...
        private void DailyTick() //this could be put into a weekly tick probably if we aren't particularly picky about which day in the week it occurs on
        {
            ...
            {
                var allEligibleKingdoms = Kingdom.All.WhereQ(x => !x.IsEliminated && (x.Culture.StringId == TORConstants.Cultures.EMPIRE || x.Culture.StringId == TORConstants.Cultures.BRETONNIA) && x.Fiefs.WhereQ(f => f.IsTown).Count() > 1);//the last city can't fall to a chaos rebellion. This needs to be investigated in more detail as it can trigger faction discontinuation or other unknown outcomes.
