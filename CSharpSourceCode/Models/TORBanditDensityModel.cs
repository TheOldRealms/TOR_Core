...
namespace TOR_Core.Models
{
    public class TORBanditDensityModel : DefaultBanditDensityModel
    {
        ...
        public override int GetMaxSupportedNumberOfLootersForClan(Clan clan)
        {
            ...
            if (timeModel.CampaignStartTime.ElapsedDaysUntilNow < TORConfig.YearsToEndEarlyCampaign * DAYS_IN_YEAR)
            {
                return TORConfig.NumberOfMaximumLooterPartiesEarly;
            }
            else if (timeModel.CampaignStartTime.ElapsedDaysUntilNow < TORConfig.YearsToEndMidCampaign * DAYS_IN_YEAR)
            {
