...
namespace TOR_Core.CharacterDevelopment.CareerSystem.Button
{
    public class MercenaryCareerButtonBehavior : CareerButtonBehaviorBase
    {
        ...
        {
            displayText = new TextObject("{=str_tor_career_button_mercenary_default}Makes the selected troop a companion.");
            if (Campaign.Current.Models.ClanTierModel.GetCompanionLimit(Hero.MainHero.Clan) <= Clan.PlayerClan.Companions.Count())
            {
                displayText = new TextObject("{=str_tor_career_button_mercenary_companion_limit}Clan companion limit has been reached.");
                return false;
            }
            ...
            {
                displayText = new TextObject("{=str_tor_career_button_mercenary_wrong_culture}Troop must be of your culture.");
                return false;
            }

            if (characterObject.IsKnightUnit() && !characterObject.IsRanged)
            {
                displayText = new TextObject("{=str_tor_career_button_mercenary_no_knightly}Only works for non-knightly units.");
                return false;
            }

            if (characterObject.IsTreeSpirit() || characterObject.IsUndead())
            {
                displayText = new TextObject("{=str_tor_career_button_mercenary_no_tree_undead}Only works for humans or elves");
                return false;
            }

            if (characterObject.Level < 26)
            {
                displayText = new TextObject("{=str_tor_career_button_mercenary_not_tier5}Troop needs to reach tier 5 or higher.");
                return false;
            }
