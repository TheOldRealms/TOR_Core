...
namespace TOR_Core.GameManagers
{
    public static class TORKeyInputManager
    {
        ...
        private static void InitializeHotKeyManager()
        {
            ...
            KeyElementBindings.AddVariationWithId(spellcastingModeKey, new TextObject("{=str_tor_spellcasting_mode_key}Quick Cast Selection Menu"), new List<GameTextManager.ChoiceTag>());

            var quickCast = context + "_" + (int)TorKeyMap.QuickCast;
            KeyElementBindings.AddVariationWithId(quickCast, new TaleWorlds.Localization.TextObject("{=str_tor_quick_cast_key}Quick Cast"), new List<GameTextManager.ChoiceTag>());

            var specialMove = context + "_" + (int)TorKeyMap.CareerAbilityCast;
            KeyElementBindings.AddVariationWithId(specialMove, new TaleWorlds.Localization.TextObject("{=str_tor_career_ability_key}Career Ability"), new List<GameTextManager.ChoiceTag>());


            var spellCastingModeDescription = new TextObject("{=str_tor_spellcasting_mode_description}1) During battle pressing Q puts you into aiming mode, a reticule will appear and you will be able to target your spells. Aiming mode is accompanied by a visual animation on your character." + "\n" +
            ...
            KeyDescriptionElement.AddVariationWithId(quickCast, new TaleWorlds.Localization.TextObject(
                "{=str_tor_quick_cast_description}Cast spells or prayers without needing to switch to spell cast mode. Warning: There is no target indication and the Spells follow predefined casting order. For targeted casting, always use the Spellcaster mode"), new List<GameTextManager.ChoiceTag>());
            KeyDescriptionElement.AddVariationWithId(specialMove, new TaleWorlds.Localization.TextObject(
                "{=str_tor_career_ability_description}Starting Career ability, like the mistform for the Vampire Counts, or Knightly strike of the Grailknight"), new List<GameTextManager.ChoiceTag>());
        }
    }
