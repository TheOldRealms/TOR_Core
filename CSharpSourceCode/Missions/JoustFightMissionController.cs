...
namespace TOR_Core.Missions
{
    public class JoustFightMissionController(CultureObject culture) : MissionLogic, ITournamentGameBehavior
    {
        ...
        private void SpawnParticipantAgent(TournamentParticipant participant, Team team, MatrixFrame frame)
        {
            ...
            vec = vec.Normalized();
            AgentBuildData agentBuildData2 = agentBuildData.InitialDirection(vec).Equipment(participant.MatchEquipment).ClothingColor1(team.Color).Banner(team.Banner).Controller(character.IsPlayerCharacter ? AgentControllerType.Player : AgentControllerType.AI);
                ...
                if (_match.IsPlayerParticipating())
                {
                    ...
                    if (IsThereAnyPlayerAgent())
                    {
                        if (Mission.IsPlayerCloseToAnEnemy(5f))
                        {
                            ...
                            MBInformationManager.AddQuickInformation(GameTexts.FindText("str_can_not_retreat", null));
                        }
