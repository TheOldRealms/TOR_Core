    ...
    public class DuelFightMissionController(Action<bool> onMissionEnd, Hero duelHero = null) : MissionLogic
    {
        ...
        public override void OnAgentRemoved(Agent affectedAgent, Agent affectorAgent, AgentState agentState, KillingBlow blow)
        {
            if (affectedAgent == Agent.Main || affectedAgent == _duelAgent)
            {
                ...
                MBInformationManager.AddQuickInformation(GameTexts.FindText("str_duel_has_ended", null));
                if (onMissionEnd != null) onMissionEnd(affectedAgent != Agent.Main);
            }
        }
        ...
        {
            canLeave = _duelEnded;
            if (!canLeave) MBInformationManager.AddQuickInformation(GameTexts.FindText("str_can_not_retreat_duel_ongoing", null));
            return null;
        }
    }
}